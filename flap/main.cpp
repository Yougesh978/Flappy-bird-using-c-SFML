...
int main() {
	...
	struct Textures {
		Texture flappy[3];         //3 to make the bird look like fly
		Texture pipe;
		Texture background;
		Texture gameover;
	} textures;
	...
	textures.pipe.loadFromFile("./images/pipe-red.png");
	...
	game.font.loadFromFile("TiltPrism-Regular.ttf");
	...
	while (window.isOpen()) {
		...
		if (game.gameState == started && game.frames % 150 == 0) {
			...
			pipes.push_back(pipeU);         // push to the array
		}


		if (game.gameState == started) {
			for (vector<Sprite>::iterator itr = pipes.begin(); itr != pipes.end(); itr++) {       // move pipes
				(*itr).move(-3, 0);
			}
		}
		...
		if (game.gameState == started) {
			for (vector<Sprite>::iterator itr = pipes.begin(); itr != pipes.end(); itr++) {
				...
				if ((*itr).getScale().y > 0) {
					px = (*itr).getPosition().x;                                 // collision detection
			...
			if (event.type == Event::Closed) {
				...
				if (game.gameState == started) {
					flappy.v = -8;
					   ...
					   game.gameState == gameover) {                        //for restart
		...
		window.draw(game.background[1]);                    // clear, draw, display
